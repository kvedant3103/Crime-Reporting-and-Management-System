...
mongoose.connect("mongodb://localhost:27017/ciudm", {
  ...
  User.findOne({
    ...
    if (!user) {
      ...
      newUserRequest.save(function(err) {
        if (!err) {
          ///////////////////////////////////////Send Email ***********************************
          let currentStatus = "Details Submitted",
            message = "",
            caseId = "";
          sendMail(req.body.email, caseId, currentStatus, message);
          res.render("home", {
            successMessage: "Request submitted Succesfully. Once your request is approved, you will be able to Login with registered credentials."
          })
        } else {
          res.send(err);
        }
    ...
    JoinRequest.findOne({
      ...
      newUser.save(function(err) {
        if (!err) {
          ...
          ////////////////////////////////////Send Mail//////////////////////****************
          let currentCaseId = "",
            currentStatus = "Account Approved",
            message = "";
    ...
    JoinRequest.findOne({
      ...
      newReject.save(function(err) {
        if (err) {
          ...
          let currentCaseId = "",
            currentStatus = "Account Rejected",
            message = req.body.reasonOfRejection;
          sendMail(userId, currentCaseId, currentStatus, message);
        }
      });
...
app.post("/login", function(req, res) {
  ...
  RejectList.findOne({
    ...
    if (rejected === null) {
      ...
      User.findOne({
        ...
        if (user !== null) {
          ...
          JoinRequest.findOne({
            ...
            if(user){
              res.render("login", {
                message: "Request to join is not accepted yet. You will be notified once it is accepted or rejected."
              });
            }
            else{
              res.render("login", {
      ...
      res.render("login", {
        message: "Sorry, your request for account is rejected with comment(s) - " + rejected.comment
      });
    }
  });
      ...
      Case.findOne({
        ...
        if (result) {
          ...
          res.render("newCase", {
            ...
            caseDetails: newCase,
            caseId: "",
            message: "ID already exists. Use another ID for this case"
          });
        } else {
          newCase.save(function(err) {
            if (!err) {
              if (requestId !== "notApplicable") {
                CaseRequest.updateOne({
                  _id: requestId
                }, {
                  status: "Case is approved and Registered"
                }, function(err) {
                  if (err) {
                    res.send(err);
                  } else {
                    ////////////////////////////////////////// Mailer Function Call///////
                    let currentStatus = "Case is approved and Registered",
                      message = req.body.caseSubjectOrTitle;
              ...
              if (req.body.radio !== null) {
                if (req.body.radio === "suspect") {
                  res.render("newSuspectForm", {
                    cid: caseId,
                    newCaseMessage: "Case registered successfully!"
                  });
                } else if (req.body.radio === "criminal") {
                  res.render("postNewCase", {
                    newCaseMessage: "Case registered successfully!",
                    failure: "",
                    profiles: "",
                    cid: caseId
                  ...
                  res.render("dashboard", {
                    dashboardMessage: "Case registered successfully!",
                    failureDashboardMessage: "",
                    userType: typeOfUser
                  });
                }
...
function sendMail(user, caseId, status, message) {
  ...
  if (user !== "user@admin.com" || user !== "user@police.com" || user !== "user@public.com" || process.env.USER !=="your@email.com") {
...
app.post("/postNewCase", function(req, res) {
  ...
  if (middleName === "" && lastName !== "") {
    ...
    Criminal.find({
      ...
      'name.lname': lastName
    }, function(err, criminals) {
      if (criminals.length === 0) {
        res.render("postNewCase", {
          profiles: criminals,
          failure: "No record found for " + firstName + " " + middleName + " " + lastName,
          newCaseMessage: "",
          cid: retainedCaseId
        });
      } else {
  ...
  if (isLoggedIn) {
    if (typeOfUser === "Admin") {
      Case.find(function(err, cases) {
        if (cases.length == 0 || err) {
          res.render("dashboard", {
            dashboardMessage: "",
            failureDashboardMessage: "No record found!",
            userType: typeOfUser
          });
        } else {
      ...
      CaseRequest.find({
        user: currentUserId
      }, function(err, userCases) {
        if (err) {
          res.render("dashboard", {
            dashboardMessage: "",
            failureDashboardMessage: "Something went wrong",
            userType: typeOfUser
          });
        } else if (userCases.length !== 0) {

          retainedSearch = userCases;
          res.render("pendingCaseList", {
            userCases: userCases,
            failure: "",
            userType: typeOfUser
          });
...
app.post("/cases", function(req, res) {
  ...
  if (typeOfOption === "caseId") {
    Case.findOne({
      _id: req.body.searchValue
    }, function(err, cases) {
      if (!cases || err) {
        res.render("caseList", {
          cases: retainedSearch,
          failure: "No record found for ID " + req.body.searchValue,
          userType: typeOfUser
        });
      } else {
        res.render("caseList", {
    ...
    Case.findOne({
      subjectOrTitle: req.body.searchValue
    }, function(err, cases) {
      if (!cases || err) {
        res.render("caseList", {
          cases: retainedSearch,
          failure: "No record found for tittle " + req.body.searchValue,
          userType: typeOfUser
        });
      } else {
        res.render("caseList", {
...
app.get("/pendingCaseRequest", function(req, res) {
  if (isLoggedIn && typeOfUser === "Admin") {
    CaseRequest.find({
      status: {
        $nin: ["Case is approved and Registered", "Case is rejected"]
      }
    }, function(err, requestedCases) {
...
app.post("/pendingCaseRequest", function(req, res) {
  ...
  if (typeOfOption === "caseId") {
    CaseRequest.findOne({
      ...
      if (!cases || err) {
        res.render("pendingCaseList", {
          userCases: retainedSearch,
          failure: "No cases found",
          userType: typeOfUser
        });
      } else {
        console.log(cases);
...
app.post("/acceptCaseRequest/:caseId", function(req, res) {
  ...
  CaseRequest.findOne({
    _id: req.params.caseId
  }, function(err, requestedCase) {
    console.log(requestedCase.user);
    res.render("newCase", {
      userType: typeOfUser,
      caseDetails: requestedCase,
...
app.post("/rejectCaseRequest/:caseId", function(req, res) {
  ...
  CaseRequest.findOneAndUpdate({
    _id: req.params.caseId
  }, {
    status: "Case is rejected",
    rejectedComment: reasonOfRejection
  }, function(err, foundCase) {
    if (err) {
      res.send(err);
    } else {
      ///////////////////////// Send Mail////////////////****************
      let currentStatus = "Case is rejected",
        user = foundCase.user,
        message = reasonOfRejection,
        caseId = foundCase.subjectOrTitle;
...
app.post("/findCaseByFilter", function(req, res) {
  ...
  if (!key) {
    res.render("profileList", {
      pageHeading: "Criminals",
      profiles: retainedSearch,
      failure: "Please select an option first"
    });
  } else {
    ...
    Criminal.find({
      ...
      if (criminals.length === 0 || err) {
        res.render("profileList", {
          pageHeading: "Criminals",
          profiles: criminals,
          failure: "No record found for " + value
        });
      } else {
        res.render("profileList", {
          pageHeading: "Criminals",
...
app.get("/findcase/:caseId", function(req, res) {
  Case.findOne({
    ...
    if (!err) {
      ...
      res.render("dashboard", {
        dashboardMessage: "",
        failureDashboardMessage: "Something went wrong. Try again later.",
        userType: typeOfUser
      });
    }
  })
...
app.get("/Criminals/:criminalId", function(req, res) {
  Criminal.findOne({
    _id: req.params.criminalId
  }, function(err, criminal) {
    if (!err) {
      res.render("profileView", {
        profileDetails: criminal,
        pageHeading: "Criminal Record",
        failure: ""
      });
    } else {
      res.render("dashboard", {
...
app.get("/Suspects/:suspectId", function(req, res) {
  Suspect.findOne({
    ...
    if (!err) {
      res.render("profileView", {
        profileDetails: suspect,
        pageHeading: "Suspect Record",
        failure: ""
      });
    } else {
      res.render("dashboard", {
    ...
    if (!err) {
      Case.findOne({
        ...
        if (!err) {
          res.render("viewCase", {
            caseDetails: foundCase,
            failure: "Suspect added successfully",
            userType: typeOfUser
          });
        }
      });
    ...
    if (!err) {
      Case.findOne({
        ...
        if (!err) {
          res.render("viewCase", {
            caseDetails: foundCase,
            failure: "Criminal added successfully",
            userType: typeOfUser
          });
        }
      });
      ...
      res.render("dashboard", {
        dashboardMessage: "",
        failureDashboardMessage: "Operation Failed.",
        userType: typeOfUser
      });
    }
  });
      ...
      if (suspects.length === 0) {
        Case.findOne({
          ...
          if (!err) {
            res.render("viewCase", {
              caseDetails: foundCase,
              failure: "No suspects in the record. (If any, add them)",
              userType: typeOfUser
            });
          }
        })
      ...
      if (criminals.length === 0) {
        Case.findOne({
          ...
          if (!err) {
            res.render("viewCase", {
              caseDetails: foundCase,
              failure: "No criminals in the record. (If any, add them)",
              userType: typeOfUser
            });
          }
        })
        ...
        if (criminals.length !== 0) {
          criminals.forEach(function(criminal) {
            ...
            if (suspect.details.height.ft !== "" && criminal.details.ft !== "") {
              ...
              let upperLimit = criminalHeightInNumbers + 2,
                lowerLimit = criminalHeightInNumbers - 2;
            ...
            let arrObj = {
              id: criminalID,
              firstName: criminal.name.fname,
              lastName: criminal.name.lname,
              crimes: criminal.typeOfCrime,
              bodyMark: sendBodyMark,
              shortAddress: criminal.address.saddress,
              matchScore: percent
            }
            result.push(arrObj);
          ...
          res.render("matchResult", {
            matchResultArray: finalResult
          });
        } else {
          res.render("profileView", {
            profileDetails: suspect,
            pageHeading: "Suspect Record",
            failure: "No Criminal record to match with"
          });
        }
      })
